"""
app.py  —  폐국 관리 시스템 · 메인 현황  (UI v3 — BI-grade)
...
.block-container { padding: 0.75rem 1.5rem 1.5rem 1.5rem; max-width: 100%; }
...
.kpi-sub {
    font-size: 11px; opacity: 0.45; margin-bottom: 10px;
}
.kpi-track {
...
.tile-sub {
    font-size: 11px; opacity: 0.4; margin-top: 1px;
}
.tile-legend {
    display: flex; gap: 12px; flex-wrap: wrap; align-items: center;
}
.leg { display: flex; align-items: center; gap: 5px; font-size: 10px; opacity: 0.6; }
...
.stat-chip .lbl { opacity: 0.5; font-weight: 400; }
...
.stTabs [data-baseweb="tab-list"] {
    gap: 0; border-bottom: 1px solid rgba(128,128,128,0.15);
}
...
.stTabs [aria-selected="true"] { border-bottom: 2px solid #185FA5; }
...
.bi-table th {
    ...
    padding: 8px 10px; border-bottom: 1px solid rgba(128,128,128,0.12);
    background: rgba(128,128,128,0.04);
}
.bi-table td {
    padding: 8px 10px; border-bottom: 1px solid rgba(128,128,128,0.07);
...
# ── 색상 팔레트 (차분하고 신뢰감 있는 톤) ─────────────────────
C = dict(
    blue   = "#1A6FC4",   # 주 강조 — 더 차분한 블루
    green  = "#2D7D46",   # 성공·달성
    amber  = "#B45309",   # 주의
    red    = "#C0392B",   # 경고
    purple = "#5B4DA0",
...
def base_layout(**kwargs):
    d = dict(
        ...
        hoverlabel=dict(bgcolor="white", bordercolor=GC,
...
def styled_xaxis(**kw):
    d = dict(tickfont=dict(size=10, color="#9CA3AF"),
             gridcolor=GC, linecolor="rgba(128,128,128,0.15)",
             showline=True, zeroline=False)
    d.update(kw); return d

def styled_yaxis(**kw):
    d = dict(tickfont=dict(size=10, color="#9CA3AF"),
             gridcolor=GC, linecolor=PLOT_BG,
             zeroline=False)
    d.update(kw); return d

# ── HTML 컴포넌트 헬퍼 ───────────────────────────────────────
def kpi_tile(label, value, sub, pct, color, color_l=None):
    pct = min(max(pct, 0), 100)
    bg = color_l or "transparent"
    return (
        ...
        '</div></div>'
    ).format(bg=bg, c=color, lbl=label, val=value, sub=sub, pct=pct)
...
def chip(label, value, color=None):
    val_html = '<span class="val" style="color:{}">{}</span>'.format(color, value) if color else '<span class="val">{}</span>'.format(value)
    return '<span class="stat-chip"><span class="lbl">{}</span>{}</span>'.format(label, val_html)

def badge(text, bg, fg):
    return '<span class="badge" style="background:{};color:{}">{}</span>'.format(bg, fg, text)
...
with st.sidebar:
    st.markdown(
        ...
        '<div style="font-size:11px;opacity:.45;margin-top:2px">04.중부 본부</div>'
    ...
    for label, href, active in [
        ...
        st.markdown(
            ...
            'background:{};font-size:12px;font-weight:{};color:{}">{}</div></a>'.format(
                href, bg, fw, clr, label),
            unsafe_allow_html=True,
        )

    st.divider()
    st.markdown('<div style="font-size:10px;font-weight:600;letter-spacing:.06em;opacity:.4;text-transform:uppercase;padding:2px 2px 6px">Off 월</div>', unsafe_allow_html=True)
    ...
    st.divider()
    st.markdown('<div style="font-size:10px;font-weight:600;letter-spacing:.06em;opacity:.4;text-transform:uppercase;padding:2px 2px 6px">데이터 파일</div>', unsafe_allow_html=True)
    for path, label in [(MAIN_FILE,"중부_원시데이터.xlsx"),(RENT_FILE,"임차_전기DB.xlsx")]:
        icon = "🟢" if path.exists() else "🟡"
        st.markdown('<div style="font-size:11px;opacity:.6;padding:2px 0">{} {}</div>'.format(icon, label), unsafe_allow_html=True)
...
with h_l:
    st.markdown(
        ...
        '<span class="badge" style="background:#FEF3C7;color:#92400E">04.중부</span>'
...
with h_r:
    r1, r2 = st.columns(2)
    with r1:
        if st.button("🔄 새로고침", key="hdr_refresh"):
            st.cache_data.clear(); st.rerun()
    with r2:
        st.button("📥 내보내기", key="hdr_export")

# ── KPI 카드 ─────────────────────────────────────────────────
k1,k2,k3,k4,k5 = st.columns(5)
kpi_data = [
    (k1, "누적 실적 (1~3월)", str(total_conf)+"개소",
     "연간 목표 {} 대비".format(ANNUAL_GOAL), pct_conf, C["blue"], C["blue_l"]),
    (k2, "임차료 절감(예상)", str(rent_conf)+"억",
     "순절감 기준 (억원)", 65, C["green"], C["green_l"]),
    (k3, "전기료 절감(예상)", str(elec_conf)+"억",
     "순절감 기준 (억원)", 40, C["green"], C["green_l"]),
    (k4, "VoC 현황", "{}건".format(voc_issued),
     "처리완료 {} · 미처리 {}건".format(voc_done, voc_open),
     (voc_open/voc_issued*100) if voc_issued else 0, C["red"], C["red_l"]),
    (k5, "Off 장비 수량", "{}대".format(eq_total),
     "1~3월 합산 (장비 Type 포함)", 55, C["teal"], C["teal_l"]),
]
...
with tab1:
    ...
    # ① 목표 대비 실적
    with col1:
        legend1 = (leg(C["blue"],"월 실적") +
                   leg(C["green"],"누계 달성률","line") +
                   leg(C["red"],"목표 100%","dash"))
        stats1  = (chip("누계", str(total_conf)+"건", C["blue"]) +
                   chip("목표", str(ANNUAL_GOAL)+"건") +
        ...
        fig1.add_trace(go.Bar(
            ...
            marker=dict(color=C["blue"], opacity=0.85, line=dict(width=0)),
        ...
        fig1.add_trace(go.Scatter(
            ...
            textfont=dict(size=10, color=C["green"], family="Inter"),
            hovertemplate="%{x} 누계달성률: %{y}%<extra></extra>",
        ), secondary_y=True)
        # 목표선
        fig1.add_hline(y=100, line_dash="dot", line_color=C["red"],
        ...
        fig1.update_xaxes(**styled_xaxis())
        fig1.update_yaxes(title_text="개소", secondary_y=False, **styled_yaxis())
        fig1.update_yaxes(title_text="달성률", secondary_y=True,
                          range=[0, 135], ticksuffix="%", **styled_yaxis(showgrid=False))
    ...
    with col2:
        ...
        fig2.update_xaxes(**styled_xaxis())
        fig2.update_yaxes(title_text="억원", secondary_y=False, **styled_yaxis())
        fig2.update_yaxes(title_text="누계", secondary_y=True,
                          ticksuffix="억", **styled_yaxis(showgrid=False))
    ...
    with col3:
        ...
        fig3.update_yaxes(title_text="대", **styled_yaxis())
        st.plotly_chart(fig3, width="stretch")
        tile_close()

    # ④ VoC 현황
    with col4:
        legend4 = (leg(C["red"],"발생") + leg(C["green"],"처리완료") +
        ...
        if voc_df.empty:
            ...
            fig4.add_trace(go.Bar(x=voc_df["월"], y=voc_df["처리완료"], name="처리완료",
                                   ...
                                   hovertemplate="%{x} 처리완료: %{y}건<extra></extra>"),
                           secondary_y=False)
...
with tab2:
    ...
    for _, row in m_sum2.iterrows():
        ...
        cumul_disp = fmt(cumul,"건") if (cumul and not pd.isna(cumul)) else '<span style="opacity:.3">—</span>'
        sbg, sfg = STATUS_MAP.get(status, ("#F3F4F6","#6B7280"))

        def sav_badge(v, bg, fg):
            return '<span style="background:{};color:{};border-radius:4px;padding:1px 7px;font-size:10px;font-weight:500">{}</span>'.format(bg,fg,v)

        rows_html.append("".join([
            "<tr>",
            "<td><b>{}</b></td>".format(mon),
            '<td style="text-align:right">{}</td>'.format(fmt(actual if actual else None,"건",C["blue"])),
            '<td style="text-align:right">{}</td>'.format(cumul_disp),
            '<td style="text-align:right">{}</td>'.format(pct_html),
            '<td style="text-align:right">{}</td>'.format(sav_badge(c_ije,"#D1FAE5","#065F46")),
            '<td style="text-align:right">{}</td>'.format(sav_badge(c_elec,"#FEF3C7","#92400E")),
            '<td style="text-align:right">{}</td>'.format(sav_badge(c_none,"#F3F4F6","#6B7280")),
            '<td style="text-align:right">{}</td>'.format(fmt(r_rent if r_rent else None,"억",C["green"])),
            '<td style="text-align:right">{}</td>'.format(fmt(r_elec if r_elec else None,"억",C["amber"])),
            '<td style="text-align:right">{}</td>'.format(fmt(r_inv  if r_inv  else None,"억",C["red"])),
            '<td style="text-align:right">{}</td>'.format(fmt(r_net  if r_net  else None,"억",C["green"])),
            '<td><span class="badge" style="background:{};color:{}">{}</span></td>'.format(sbg,sfg,status),
            "</tr>",
        ]))

    HEADERS = ["월","실적","누계","누계달성률","임차+전기","전기만","절감없음",
               "임차료절감","전기료절감","투자비","순절감","상태"]
    st.markdown(
        '<table class="bi-table"><thead><tr>'
        + "".join('<th style="text-align:{}">{}</th>'.format("left" if i==0 else "right",h) for i,h in enumerate(HEADERS))
    ...
    with td1:
        tile_open("누계 달성률 추이", "월별 누계 ÷ 연간 목표 {}".format(ANNUAL_GOAL),
                  leg(C["blue"],"달성률","line") + leg(C["red"],"목표 100%","dash"), "")
        valid2 = m_sum2[m_sum2["누계달성률"].notna()]
        fig_p  = go.Figure()
        fig_p.add_traces([
            go.Scatter(x=valid2["월"], y=valid2["누계달성률"],
                       mode="lines+markers+text",
                       line=dict(color=C["blue"], width=2.5, shape="spline"),
                       fill="tozeroy", fillcolor="rgba(26,111,196,0.06)",
        ...
        fig_p.update_xaxes(**styled_xaxis())
        fig_p.update_yaxes(range=[0,125], ticksuffix="%", **styled_yaxis())
    ...
    with td2:
        ...
        fig_s   = go.Figure(go.Pie(
            ...
            marker=dict(
                colors=[SAV_C.get(l,C["gray"]) for l in sav_cnt.index],
...
with tab3:
    ...
    with ta1:
        ...
        fig_a1.update_yaxes(title_text="건수", **styled_yaxis())
    ...
    with ta2:
        ...
        fig_a2 = go.Figure(go.Bar(
            ...
            marker=dict(
                color=[BIZ_C.get(b,C["gray"]) for b in bs["사업유형"]],
                opacity=0.85, line=dict(width=0),
            ),
            text=["{}억".format(v) for v in bs["임차료절감"]],
            textposition="outside",
            textfont=dict(size=11),
            hovertemplate="%{y}: %{x}억<extra></extra>",
        ))
        fig_a2.update_layout(**base_layout(height=240, margin=dict(l=2,r=50,t=6,b=2)))
        fig_a2.update_xaxes(title_text="억원", **styled_xaxis())
        fig_a2.update_yaxes(**styled_yaxis())
    ...
    for _, row in bs.iterrows():
        ...
        rows_biz.append("".join([
            ...
            '<td style="text-align:right;color:{}">{}억</td>'.format(C["red"],"{}".format(row["투자비"]) if row["투자비"] else "—"),
    ...
    if not opt_inv.empty:
        ...
        st.info("📌 **최적화후폐국** — 투자비 발생 {}건 · 평균 BEP **{}** · 순절감 합계 {}억원 (투자비 {}억 차감 후)".format(
            len(opt_inv), bep_s, net_sum, inv_sum))